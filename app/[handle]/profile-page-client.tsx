...
export default function ProfilePageClient({
  ...
  const fetchUserData = async (isRetry = false) => {
    try {
      ...
      // Call RPC directly - no intermediate API needed
      const response = await fetch("https://api2.onfire.so/rpc/get_public_user_profile", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Accept": "application/json",
        },
        body: JSON.stringify({ p_username: params.handle }),
      })
      ...
      // Map RPC response to UserData format
      const userData: UserData = {
        ...
        businessDescription: `Level ${user.level} user`,
        bio: profile?.about || user.bio || "Welcome to my OnFire profile!",
        ...
        products: (data.products || []).map((p: any) => ({
          ...
          title: p.name,
          price: `${p.currency || 'USD'} ${p.price?.toFixed(2) || '0.00'}`,
          compareAtPrice: p.compare_at_price ? `${p.currency || 'USD'} ${p.compare_at_price.toFixed(2)}` : undefined,
          image: p.cover_image || (p.images?.find((i: any) => i.is_primary)?.image_url) || (p.images?.[0]?.image_url) || "/placeholder.svg",
          description: p.description || "",
          availability: "available",
          isOnSale: p.compare_at_price && p.compare_at_price > p.price,
          discountPercentage: p.compare_at_price && p.compare_at_price > p.price
        ...
        services: (data.services || []).map((s: any) => ({
          ...
          title: s.name,
          price: `${s.currency || 'USD'} ${s.price?.toFixed(2) || '0.00'}`,
          compareAtPrice: s.compare_at_price ? `${s.currency || 'USD'} ${s.compare_at_price.toFixed(2)}` : undefined,
          image: s.cover_image || (s.images?.find((i: any) => i.is_primary)?.image_url) || (s.images?.[0]?.image_url) || "/placeholder.svg",
          description: s.description || "",
          availability: "available",
          isOnSale: s.compare_at_price && s.compare_at_price > s.price,
          discountPercentage: s.compare_at_price && s.compare_at_price > s.price
            ? Math.round((1 - s.price / s.compare_at_price) * 100)
            : 0,
          inStock: true,
          tags: [],
        })),
        communities: (data.communities || []).map((c: any) => ({
          ...
          title: c.name,
          price: "Free",
          image: c.cover_image || c.avatar || "/placeholder.svg",
          description: c.description || "",
          availability: c.privacy,
          isOnSale: false,
      ...
      // Set products data from the mapped response
      setProductsData({
